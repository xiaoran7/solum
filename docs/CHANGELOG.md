...
- **Solum 0.2.2（Android versionCode 1012，2026-08-13）——账户外壳、访客边界与工作台 UI 重构。** 首次完成隐私同意后直接提供注册、登录和访客使用三条路径；账户身份提升到桌面侧栏与移动顶栏，完整能力保留可见并以锁定入口解释价值。访客只保留今天、本地计划、记忆台账、行为记录与本地搜索；云端推理、主动建议、复盘、穿戴、通知智能、邮箱、资料、组件、同步和福利告警必须登录。Rust 壳层在 Tauri IPC 和 Guard 工具入口强制校验账号会话，旧直连 API Key、旧 token 或前端绕路均不能解锁完整能力。排版改为固定字级和零字距，标题、圆角、导航密度与动效时间线按工作台场景收紧；访客隐私页只保留账号能力说明与始终可读的完整政策，不暴露不可用表单。
  - 验证：`cargo test --workspace` 全绿（solum-core 311 项、solum-app 16 项、闭环 4 项、relay 7 项等），Clippy `-D warnings`、fmt、前端脚本/id/账户 IPC 边界/隐私视图结构/禁用样式检查和 `git diff --check` 通过。真实浏览器在桌面与 390×844 验证首次身份三选一、访客进入、今日简报锁定回流、隐私页启动恢复和账户面板，无横向溢出、应用控制台零错误。arm64 Release APK 为 27,511,553 字节，versionName 0.2.2 / versionCode 1012，仅含 arm64-v8a，APK Signature Scheme v2 有效，SHA-256 `42EA0C9CF3174B4659C79D8354E18F9BA92E8AB32A74E75AB7117BA9DFF21FBA`；release 源库、Gradle jniLibs 与 APK 内 `.so` 三方 SHA-256 均为 `36216A70FF368A8B046F3743037F17CF16F71DAB074913E8D0768EED3BC41FC0`。当前无在线 Android 设备，未做真机安装与中心账号登录运行验证。

- **SQLite 到 PostgreSQL 中心库的可回滚迁移工具（2026-08-13）**：新增只读旧账号库与 relay 库的一次性导入器，在单个事务中映射用户名租户到不可变 UUID，保留密码哈希、刷新会话和 blob/告警原始游标序号；目标库非空、租户归属不唯一或 schema 不兼容时拒绝导入并回滚，避免中心化切换时丢失登录状态或 Android 告警连续性。
