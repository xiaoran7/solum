...
- 所有云端调用的唯一出口，职责：
  ...
  8. **统一账号身份（2026-08-12，由账号代理扩展）**：直连厂商 API Key 之外的第二条云端通路仍是登录自建 `server/`（solum-cloud，Node 单进程，与 Solum Harmony 共用同一契约）后请求 `{server}/v1/ai/chat/completions`，第三方 API Key 只存在服务端环境变量。`solum-core::account` 会话落 `solum-account.json`（gitignored，密码只用于登录请求与本地恢复信封解密、不落盘），访问令牌 15 分钟过期、401 时刷新一次并轮换刷新令牌。该访问令牌同时是 §3.8 同源同步接口的租户身份证明：服务端验签后只以令牌 `sub` 选择租户，拒绝客户端自报用户名。**身份关联不等于解密权**：同步主密钥随机生成并只在客户端解封/保存，账号服务与同步服务均拿不到同步明文。登录期间账号代理优先于直连配置；退出登录回落直连配置或纯离线，同步与福利告警则暂停到重新登录，所有本机能力不受影响。
